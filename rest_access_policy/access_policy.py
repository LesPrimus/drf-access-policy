...
class Statement:
    ...
    # Fields forced read-only (by FieldAccessMixin) when the principal matches.
    read_only_fields: Union[List[str], str] = field(default_factory=list)

    def __post_init__(self):
        permitted = ("allow", "deny")

        if self.effect not in permitted:
            raise ValueError(f"effect must be one of {permitted}")

        # Normalize scalar values to lists so the engine can rely on a single,
        # consistent shape (previously done ad hoc while converting to dicts).
        self.principal = self._as_list(self.principal)
        self.action = self._as_list(self.action)
        self.condition = self._as_list(self.condition)
        self.condition_expression = self._as_list(self.condition_expression)
        self.read_only_fields = self._as_list(self.read_only_fields)

    @staticmethod
    def _as_list(value: Union[List[str], str]) -> List[str]:
...
class AccessPolicy(permissions.BasePermission):
    ...
    def _evaluate_statements(
        ...
        statements = self._validate_statements(statements)
    ...
    @staticmethod
    def _validate_statements(statements: List[Statement]) -> List[Statement]:
        """
        Ensure every statement is a ``Statement`` instance. Passing statements
        as plain dicts is no longer supported.
        """
        for statement in statements:
            if not isinstance(statement, Statement):
                raise AccessPolicyException(
                    "Statements must be 'Statement' instances; got "
                    f"'{type(statement).__name__}'. Passing statements as dicts "
                    "is no longer supported — wrap them in Statement(...)."
                )

        return statements

    @classmethod
    def _get_user_principals(cls, request) -> set:
        """
        Return the set of principal identifiers the requesting user satisfies
        (e.g. ``{"authenticated", "id:5", "group:cooks"}``). Group values are
        resolved once so callers can match many statements cheaply.
        """
        ...
        principals.add(cls.id_prefix + str(user.pk))

        for user_role in cls().get_user_group_values(user):
            principals.add(cls.group_prefix + user_role)
    ...
    def _get_statements_matching_principal(
        ...
        user_principals = cls._get_user_principals(request)
        matched = []

        for statement in statements:
            principals = statement.principal

            if "*" in principals or not user_principals.isdisjoint(principals):
    ...
    def _get_statements_matching_action(
        ...
        for statement in statements:
            ...
            if action in actions or "*" in actions:
                matched.append(statement)
            elif http_method in actions:
                matched.append(statement)
            elif "<safe_methods>" in actions and request.method in SAFE_METHODS:
                matched.append(statement)
    ...
    def _get_statements_matching_conditions(
        ...
        *,
        ...
        for statement in statements:
            conditions = getattr(statement, element_key)
